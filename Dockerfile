...
# install git-crypt
RUN curl -L -o /tmp/git-crypt.zip https://www.agwa.name/projects/git-crypt/downloads/git-crypt-0.6.0.tar.gz
...
RUN cd git-crypt-0.6.0.tar.gz && make && make install PREFIX=/usr/local
